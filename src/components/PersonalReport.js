...
export const PersonalReportCard = memo(({ player, rank, onClose, onViewFull, allScores, allPutts, allUps, pars, completedHoles, gameMode, t, getMedal }) => {
  ...
  const getPgaScoreClass = (score, par) => {
    ...
    if (score <= par * 2) return 'pga-triple';
    return 'pga-quad';
  };
                ...
                <div className="flex justify-between">
                  {front9Details.map(d => (
                    <div key={d.hole} className="flex justify-center">
                      <div className={getPgaScoreClass(d.score, d.par)}>{d.score}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-3">
            ...
            <div className="flex justify-around">
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg" style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)' }}>
                  <span className="text-2xl font-extrabold text-white">{birdies}</span>
                </div>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-xs font-bold">{t('parLabel')}</span>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg" style={{ background: 'linear-gradient(135deg, #9ca3af 0%, #6b7280 100%)' }}>
                  <span className="text-2xl font-extrabold text-white">{parsCount}</span>
                </div>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-bold">{t('bogey')}</span>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg" style={{ background: 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)' }}>
                  <span className="text-2xl font-extrabold text-white">{bogeys}</span>
                </div>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">{t('doubleplus')}</span>
...
// ========== Personal Full Detail Modal ==========
export const PersonalFullDetailModal = memo(({ player, rank, onClose, onBack, allScores, allPutts, allUps, pars, completedHoles, gameMode, t, getMedal }) => {
  const details = completedHoles.map(hole => {
  ...
  const renderTable = (holeDetails, label, bgClass) => (
    <>
      <div className={`px-3 py-2 ${bgClass} text-xs font-semibold sticky top-0`}>{label}</div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col">
      ...
      <div className="relative flex flex-col bg-white m-3 mt-12 mb-3 rounded-2xl shadow-2xl overflow-hidden" style={{ maxHeight: 'calc(100vh - 60px)', animation: 'cardAppear 0.25s ease-out' }}>
        ...
        <div className="flex-1 overflow-auto">
          {front9Details.length > 0 && renderTable(front9Details, `${t('front9')} OUT`, 'bg-green-50 text-green-700')}
          {back9Details.length > 0 && renderTable(back9Details, `${t('back9')} IN`, 'bg-blue-50 text-blue-700')}
        </div>

        {/* Bottom stats */}
        <div className="flex-shrink-0 bg-gray-100 p-3 border-t" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
          <div className="grid gap-2 text-center" style={{ gridTemplateColumns: `repeat(${2 + (isWin123 ? 1 : 0)}, 1fr)` }}>
